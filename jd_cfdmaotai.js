/*
京喜财富岛提现
cron 59 11,12,23 * * * jd_cfdmaotai.js
更新时间：2021-7-20
...
[task_local]
#京喜财富岛提现
59 11,12,23 * * * https://raw.githubusercontent.com/Aaron-lv/sync/jd_scripts/jd_cfdmaotai.js, tag=京喜财富岛提现, img-url=https://raw.githubusercontent.com/58xinian/icon/master/jxcfd.png, enabled=true

================Loon==============
[Script]
cron "59 11,12,23 * * *" script-path=https://raw.githubusercontent.com/Aaron-lv/sync/jd_scripts/jd_cfdmaotai.js,tag=京喜财富岛提现

===============Surge=================
京喜财富岛提现 = type=cron,cronexp="59 11,12,23 * * *",wake-system=1,timeout=3600,script-path=https://raw.githubusercontent.com/Aaron-lv/sync/jd_scripts/jd_cfdmaotai.js

============小火箭=========
京喜财富岛提现 = type=cron,script-path=https://raw.githubusercontent.com/Aaron-lv/sync/jd_scripts/jd_cfdmaotai.js, cronexpr="59 11,12,23 * * *", timeout=3600, enable=true
 */
...
function cashOutQuali() {
  return new Promise((resolve) => {
    $.get(taskUrl(`user/ExchangePrize`, `dwType=1&dwLvl=44&ddwPaperMoney=100&strPoolName=jxcfd2_exchange_sw1_2021&strPgUUNum=${token['farm_jstoken']}&strPgtimestamp=${token['timestamp']}&strPhoneID=${token['phoneid']}`), (err, resp, data) => {
      try {
        if (err) {
          console.log(`${JSON.stringify(err)}`)
          console.log(`${$.name} maotai API请求失败，请检查网路重试`)
        } else {
